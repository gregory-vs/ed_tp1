...
struct Interval { double inicio, fim; };
...
void generateScene(Object obj_list[], int obj_count, Scene *scene) {
    ...
    for (int i = 0; i < obj_count; i++) {
        ...
        for (int j = 0; j < covered_count; j++) {
            ...
            if (ci >= right) break;
            if (ci > left)
                outputs[output_count++] = Output(scene->getTime(), obj_list[i].getId(), left, min(right, ci));
            if (cf > left) left = cf;
            if (left >= right) break;
        }
...
int main() {
    ifstream file("testes/inputs2.txt");
    if (!file.is_open()) {
    ...
    while (file >> type) {
        if (type == 'O') {
            ...
            Scene *scene = new Scene(scene_time);
